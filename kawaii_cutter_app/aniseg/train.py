...
class AnimeSegmentation(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # print({"val/precision": pre_m, "val/recall": rec_m, "val/f1": f1_m, "val/mae": mae_m})
        wandb.log({"val/precision": pre_m.item(), "val/recall": rec_m.item(), "val/f1": f1_m.item(), "val/mae": mae_m.item()})
...
def main(opt):
    ...
    trainer = Trainer(precision=32 if opt.fp32 else 16, accelerator=opt.accelerator,
                      ...
                      benchmark=opt.benchmark, accumulate_grad_batches=opt.acc_step,
                      check_val_every_n_epoch=opt.val_epoch, log_every_n_steps=opt.log_step,
                      strategy="ddp_find_unused_parameters_false" if opt.devices > 1 else 'ddp',
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bg-ext', type=str, default='.png',
    ...
    wandb.init(project="aniseg", entity="kyttang", config=opt, name="testrun")
    print(opt)
